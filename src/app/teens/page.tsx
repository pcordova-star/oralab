...
import { Activity, Clock, Play, CheckCircle2, AlertCircle, Timer, Volume2 } from "lucide-react";
import { PROTOCOLS, Appointment, ExamType, ProtocolStep } from "@/app/lib/types";
...
export default function TeensPage() {
  ...
  // Setup sound alert (simulated or real if possible)
  useEffect(() => {
    // We can't easily play sounds without user interaction or complex setup, 
    // but we can provide a visual "pulse" instead.
  }, []);
  ...
  useEffect(() => {
    ...
    if (timerActive && timeLeft > 0) {
      ...
      // Logic for alert sound would go here
    }
  ...
  const handleNextStep = () => {
    if (isLastStep) {
      ...
      // Update local state to remove patient from waiting list
